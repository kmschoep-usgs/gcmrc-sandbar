...
class AreaVolumeCalcsVw(CSVResponseMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if parameter_type == 'area2d':
            ...
            c_df0_float['Total Channel'] = c_df0_float['Channel']
            c_df1 = c_df0_float[['date', 'Total Channel']]
            if sr_exists:
                ...
                sr_col_names = col_name[0:]
                eddy_total_param = 'Eddy Total'
                col_names += (eddy_total_param,)
                if eddy_df_srs_len == 1:
                    df_sr = eddy_df_srs[0]
                    df_sr['Eddy Total'] = df_sr[sr_col_names[0]]
                elif eddy_df_srs_len == 2:
                    df_sr = pd.merge(eddy_df_srs[0], eddy_df_srs[1], how='outer', on='date')
                    df_sr['Eddy Total'] = df_sr.apply(sum_two_columns, axis=1, col_x=sr_col_names[0], col_y=sr_col_names[1])
                else:
                    raise Exception('Getting the separation/reattachment data went wrong...')
                e_df1 = df_sr[list(col_names)]
            else:
                e_df0_float['Eddy Total'] = e_df0_float['Eddy']
                e_df1 = e_df0_float[['date', 'Eddy Total']]
            ec_merge = pd.merge(e_df1, c_df1, how='outer', on='date')
            ec_merge['Total Site'] = ec_merge.apply(sum_two_columns, axis=1, col_x='Eddy Total', col_y='Channel Total')
            ...
            c_df0_float = c_df0.applymap(convert_to_float)
            c_df0_float['Channel Total'] = c_df0_float.apply(create_dygraphs_error_str, axis=1, low='c_low', med='c_med', high='c_high')
            c_df1 = c_df0_float[['date', 'Channel Total']]
            sr_col_names = col_name[0:]
            sr_eddy_low = 'sr_eddy_low'
            sr_eddy_med = 'sr_eddy_med'
            sr_eddy_high = 'sr_eddy_high'
            eddy_total_param = 'Eddy Total'
            eddy_col_names = (sr_eddy_low, sr_eddy_med, sr_eddy_high, eddy_total_param)
            if sr_exists:
                sr_ids = get_sep_reatt_ids(site.id)
                eddy_df_srs = []
                sr_col_names = tuple()
                for sr_id in sr_ids:
                    ...
                    df_sr[col_name] = df_sr.apply(create_dygraphs_error_str, axis=1, low='e_low', med='e_med', high='e_high') # the dygraphs error string for one of the separation/reattachment sandbars
                    eddy_df_srs.append(df_sr)
                eddy_df_srs_len = len(eddy_df_srs)
                col_names += (sr_eddy_low, sr_eddy_med, sr_eddy_high, eddy_total_param)
                if eddy_df_srs_len == 1:
                    df_sr = eddy_df_srs[0]
                    ...
                    df_sr[eddy_total_param] = df_sr[sep_reatt_col] # the dygraphs error string for combined separation/reattachment sandbars
                elif eddy_df_srs_len == 2:
                    ...
                    df_sr[eddy_total_param] = df_sr.apply(create_dygraphs_error_str, axis=1, low='sr_eddy_low', med='sr_eddy_med', high='sr_eddy_high') # the dygraphs error string for combined separation/reattachment sandbars
                else:
                    raise Exception('Getting the separation/reattachment data went wrong...')
                ...
                e_df0_float[eddy_total_param] = e_df0_float.apply(create_dygraphs_error_str, axis=1, low=sr_eddy_low, med=sr_eddy_med, high=sr_eddy_high) # the dygraphs error string if separation/reattachment doesn't apply 
                full_col_names = ('date',) + eddy_col_names
            ...
            ec_merge['Total Site'] = ec_merge.apply(create_dygraphs_error_str, axis=1, low='ec_low', med='ec_med', high='ec_high') # this is eddy + channel
